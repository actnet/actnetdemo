...
class SAEClient(object):
    ...
    def activity_search_by_name(self, name):
        r = interface_pb2.ActivitySearchRequest()
        r.name = name
        response = pbrequest(self.endpoint, "ActivitySearchByName", r)
        er = interface_pb2.ActivitySearchResponse()
        er.ParseFromString(response)
        return er

    def similar_search_by_name(self, name):
        r = interface_pb2.ActivitySearchRequest()
        r.name = name
        response = pbrequest(self.endpoint, "ActivitySearchSimilar", r)
        er = interface_pb2.ActivitySearchResponse()
        er.ParseFromString(response)
        return er

    def activity_search_by_city(self, name):
        r = interface_pb2.ActivitySearchRequest()
        r.name = name
        response = pbrequest(self.endpoint, "ActivitySearchCity", r)
        er = interface_pb2.ActivitySearchResponse()
        er.ParseFromString(response)
        return er

    def user_search_by_activity(self, name):
        r = interface_pb2.ActivitySearchRequest()
        r.name = name
        response = pbrequest(self.endpoint, "ActivitySearchUser", r)
        er = interface_pb2.ActivitySearchResponse()
        er.ParseFromString(response)
        return er

    def weibo_search_by_activity(self, name):
        r = interface_pb2.ActivitySearchRequest()
        r.name = name
        response = pbrequest(self.endpoint, "WeiboSearchByActivity", r)
        er = interface_pb2.ActivitySearchResponse()
        er.ParseFromString(response)
        return er

    def sentiment_search_by_activity(self, name):
        r = interface_pb2.ActivitySearchRequest()
        r.name = name
        response = pbrequest(self.endpoint, "SentimentSearchByActivity", r)
        er = interface_pb2.ActivitySearchResponse()
        er.ParseFromString(response)
        return er

    def center_search_by_activity(self, name):
        r = interface_pb2.ActivitySearchRequest()
        r.name = name
        response = pbrequest(self.endpoint, "CenterSearchByActivity", r)
        er = interface_pb2.ActivitySearchResponse()
        er.ParseFromString(response)
        return er

    def instance_search_by_activity(self, name):
        r = interface_pb2.ActivitySearchRequest()
        r.name = name
        response = pbrequest(self.endpoint, "ListSearchByActivity", r)
        er = interface_pb2.ActivitySearchResponse()
        er.ParseFromString(response)
        return er

    def activit_search_all_by_name(self, name):
        r = interface_pb2.ActivitySearchRequest()
        r.name = name
        response = pbrequest(self.endpoint, "ActivitySearchAllByName", r)
        er = interface_pb2.ActivitySearchResponse()
        er.ParseFromString(response)
        return er
